...
obs_dates_1 = [
    (date(2025, 10, 29), date(2025, 11, 13)),  # 28 Sep obs, 13 Oct payment
    ...
    (date(2026, 7, 28), date(2026, 7, 28)),  # Final fixing = payment
]
...
for i, (obs_date, pay_date) in enumerate(obs_dates_1):
    sched = ObservationSchedule(
        ...
        coupon_rate_period=0.037 / 12,  # ~3.14% annual / 12 months
...
cms_10y = Underlying(
    ...
    yahoo_ticker=None,
    reference_level=3.41,  # As of trade date
...
for year in range(1, 11):
    ...
    sched2 = ObservationSchedule(
        ...
        autocall_level=None,  # No autocall on this product
...
for q in range(1, 5):
    obs_date = date(2025, 7 + q*3, 13)
    if obs_date.month > 12:
        obs_date = date(2026, obs_date.month - 12, 13)
    sched3 = ObservationSchedule(
        product_id=prod3.id,
        observation_date=obs_date,
        payment_date=obs_date + timedelta(days=5),
        autocall_level=None,
        coupon_level_low=2.85,
        coupon_level_high=4.40,
        coupon_rate_period=0.092 / 4,  # 9.2% annual / 4 quarters
        status="pending",
    )
    db.add(sched3)
db.commit()
